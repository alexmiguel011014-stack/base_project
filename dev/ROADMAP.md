...
## 49. `/newproject` removido do escopo (2026-09-07)
...
**Efeito colateral descoberto e corrigido no mesmo `/execgoals`**: adicionar `GOALS 12` (número
usado na hora, na sessão local) como segundo plano ativo em `GOALS.md` (necessário pra planejar
este item + o auto-archive abaixo) quebrou um teste que assumia "sempre exatamente 1 plano
ativo" — corrigido junto (`dev/tests/validate-goals-structure.test.js`'s
`archivedPlans`/active-plan-count agora derivam do `dev/goals-archive/README.md` e do
`## Active plans` do próprio `GOALS.md` em vez de lista/número hardcoded). **Renumerado pra
GOALS 13 no merge com o GitHub (item 51 abaixo)** — outra sessão pegou o mesmo número pra um
plano completamente diferente (`Usage Efficiency & Quality Loop`, item 46-48/50 acima); detalhe
completo em
[goals-13-command-suite-cleanup-and-goal-archive-automation.md](goals-archive/goals-13-command-suite-cleanup-and-goal-archive-automation.md).
**Achado adicional durante a instalação de verificação**:
...
## 50. Harness determinístico sem dependência externa (2026-09-08)
