...
3. For each finding, ordered critical first:
   - Plan the fix. For anything beyond a trivial one-line change, use the `architect`
     subagent first, then `coder` to apply it — same workflow as any other non-trivial
     change. Trivial fixes (add a missing `.gitignore` line, create a missing
     `.env.example`) can be applied directly.
...
6. Never commit automatically — if fixes are ready, mention that `reviewer` can prepare a
   commit message, same as any other change, but wait for explicit confirmation.
