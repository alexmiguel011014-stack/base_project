...
- Dead files: generated artifacts that should be gitignored but are committed, empty files/folders, files with no incoming reference. Verify with a real grep before calling something dead.
...
STEP 3 — For each finding, state the concrete evidence (the grep that found nothing, the two near-duplicate files, the path that breaks convention). Severity: `high` (misleading or duplicated logic), `medium` (clutter that slows navigation), `low` (cosmetic).
