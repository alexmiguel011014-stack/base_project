...
2. Read `~/.config/opencode/base_project/references/project-standards.md` — this is the
   shared checklist of what a well-formed project looks like (identity, version control,
...
4. Mention relevant base_project plugins from `~/.config/opencode/base_project/plugins.json`
   only if `recommend_if` clearly matches what the user described (e.g. they said "web
   app with a database" → mention the relevant MCP) — do not dump the whole catalog.
