...
4. If `<base_project-repo>/dev/scripts/doctor.js` exists (the repository recorded at `~/.base_project/repo-path.txt`), run it for the current project and treat its output as evidence, not as the entire audit.
